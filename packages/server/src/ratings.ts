import {
  ...
  export async function updateRatings(
    ...
    var glicko_outcome;
    var outcome_player_black;
    var outcome_player_white;
    ...
    const new_game_history_player_black = updatedGameHistory(db_player_black, db_player_white.ranking[variant], outcome_player_black, variant);
    const new_game_history_player_white = updatedGameHistory(db_player_white, db_player_black.ranking[variant], outcome_player_white, variant);
    ...
    const glicko_player_black = await getGlickoPlayer(db_player_black.ranking[variant]);
    const glicko_player_white = await getGlickoPlayer(db_player_white.ranking[variant]);

    matches.push([glicko_player_black, glicko_player_white, glicko_outcome]);
  
    ranking.updateRatings(matches);
  
    const player_black_new_ranking = userRankingFromGlickoPlayer(db_player_black, glicko_player_black, variant);
    const player_white_new_ranking = userRankingFromGlickoPlayer(db_player_white, glicko_player_white, variant);
...
async function getGlickoPlayer(db_player_ranking: UserRanking): Promise<Player> {
...
function userRankingFromGlickoPlayer(player: UserResponse, glicko_player: Player, variant: string): UserRankings {
  var player_ranking = player.ranking;
...
function updatedGameHistory(player: UserResponse, opponentRating: UserRanking, game_result: string, variant: string): GameResults {

  var player_game_history = player.gameHistory

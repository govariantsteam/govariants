import { GameResponse, AbstractGame, UserRanking } from "@ogfcommunity/variants-shared";
...
export async function updateRatings(game: GameResponse, game_obj: AbstractGame<object, object>){
    ...
    var glicko_player_black; 
    var glicko_player_white;

    if (db_player_black_ranking == undefined) {
        glicko_player_black = ranking.makePlayer(1500, 350, 0.06);
    } else {
        glicko_player_black = ranking.makePlayer(db_player_black_ranking.rating, db_player_black_ranking.rd, db_player_black_ranking.vol);
    }

    if (db_player_white_ranking == undefined) {
        glicko_player_white = ranking.makePlayer(1500, 350, 0.06);
    } else {
        glicko_player_white = ranking.makePlayer(db_player_white_ranking.rating, db_player_white_ranking.rd, db_player_white_ranking.vol);
    }
